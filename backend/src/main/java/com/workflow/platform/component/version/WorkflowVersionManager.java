...
public class WorkflowVersionManager {
	 ...
	 * 创建新版本
	 */
	// public WorkflowVersionEntity createVersion(String workflowId, WorkflowDTO
	// workflow,
	...
	//
	// // 保存标签
	// version.setTags(version.getTags() == null ? new ArrayList<>() :
	// version.getTags());
	...
	//// stats.setAverageVersionInterval(duration / Math.max(1, versions.size() -
	// 1));
	//
	// // 大小统计
	// long totalSize =
	// versions.stream().mapToLong(WorkflowVersionEntity::getDataSize).sum();
	...
	// private String prepareVersionData(WorkflowDTO workflow, WorkflowVersionEntity
	// previousVersion) {
	...
	// private void calculateVersionActivity(VersionStatistics stats,
	// List<WorkflowVersionEntity> versions) {
	// // 计算版本活跃度
	// if (versions.size() < 2) {
