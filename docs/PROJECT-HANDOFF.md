...
**M4**：
2. **3D 视图 + 大屏**：3d-force-graph 粒子流向、深色科技风投屏
3. **排查模式 3D 深化**、流向动画

**远期**：
4. PostgreSQL 实机验证（方言已就绪待实机）、数据池接 Oracle（新增方言+驱动）
5. 自动血缘解析、AI 辅助排查、告警渠道（钉钉/企微）、自动动作总开关与可回滚
7. 制造场景模板、开放 API 生态
...
**当前任务 = M3 剩余（CMDB 连接器 / 开放 API）+ M4（3D / 大屏）**。M0~M3 择路核心 + RBAC 安全收紧均已交付并推送到 GitHub（后端 61 测试全绿）。
新会话第一步：读本文档 → `cd backend && mvn spring-boot:run` 起服务 → 浏览器打开 http://localhost:5173 先登录（admin/admin123），从 CMDB 连接器或 3D/大屏开始。
前端已完整可用：登录页 + 关系网画布（排查模式/路径查询/影响面）+ 工作台看板 + 告警中心 + 检测点 + 配置版本 + 数据接入，全部真实数据、RBAC 受控。
