...
### M4（✅ 已实现并推送，渲染效果待浏览器复核）
- [x] **3D 视图**（`/3d`）：3d-force-graph 立体关系网——节点按类型/等级配色缩放、边粒子流向动画+方向箭头、点击高亮相邻、路线下拉高亮、自动旋转/视角重置（依赖 `3d-force-graph`）
- [x] **深色科技大屏**（`/bigscreen`）：16:9 投屏风——KPI 卡 + 实例趋势 CSS 柱状 + TOP 慢节点 + 实时告警 + 流程状态，30s 轮询
- [x] 路由 + SideNav 导航项（「3D 视图」「大屏」）；`npm run build` 全绿
- ⚠️ 3D 与大屏的**视觉效果需浏览器实测**（无头环境无法截图验证）；布局/数据接线/构建已验证
...
M4（3D 视图 + 深色大屏）已实现并推送（**渲染效果待浏览器复核**）。剩余按价值排序：

**补充功能**：
1. **开放 API 生态**：Token 管理/轮换页；Excel/日志/IoT 连接器类型
2. **路线对比页**、排查模式 3D 深化、流向动画调优
3. **定时检测调度**（Spring @Scheduled 轮询检测点）、邮件/钉钉通知渠道、自动动作总开关与可回滚
4. PostgreSQL 实机验证（方言已就绪待实机）、数据池接 Oracle（新增方言+驱动）

**远期**：
5. 自动血缘解析、AI 辅助排查、制造场景模板
6. **图来源 G1~G5**（引擎/大模型/人工三路线，设计已定稿）——三层视图定型→前端原型→引擎最小可行版→大模型接入层→校正闭环
7. 制造场景模板、开放 API 生态
...
**当前任务 = 补充功能（开放 API 生态 / 定时检测 / 通知渠道 / 路线对比）+ 远期（图来源 G1~G5 等）**。M0~M4 全部里程碑（含 RBAC、M3 补充、3D/大屏）均已交付并推送到 GitHub（后端 76 测试全绿）。
新会话第一步：读本文档 → `cd backend && mvn spring-boot:run` 起服务 → 浏览器打开 http://localhost:5173 登录（admin/admin123）→ 左导航看 **3D 视图 / 大屏**（M4，渲染效果需浏览器复核）。
前端已完整可用：登录 + 关系网画布（排查/路径/影响面）+ 3D 视图 + 大屏 + 看板 + 告警 + 检测点 + 版本 + 数据接入（DB+CMDB）+ 系统管理（运行信息/开放 API 卡）。
