...
- **图来源多路线 + 大模型辅助（v1.2 新增决策，2026-08-16）**：新增「自动/半自动分析出图」能力——图来源三条路线（引擎分析 / 大模型分析 / 人工创建），入口处选择；引擎分析后由用户在前端**主动选择**是否加一道大模型细化（够用直接校正 / 不够加大模型 / 太乱作废重来）；统一人工校正为准；大模型通过可插拔接入层随意切换；引擎定骨架、大模型管语义、人拍板，不接大模型也照跑。**详见文档书第 15 章**（「三层视图」与「校正闭环沉淀」留待后续讨论）
...
- ⏸ 延后（诚实说明）：**Security RBAC 强制**（前端无登录页，强制 JWT 会锁死演示；基础设施已就绪，待前端登录页做好后收紧）、CMDB 连接器 / 开放 API / 流向动画 / 3D（远期）
...
M2 收尾 + M3 择路核心（路径查询/影响面/等级干预/工单流转/版本留痕）已交付。

**M3 剩余**：
1. **CMDB 连接器 / 开放 API**：数据池接 CMDB（HTTP/API 型连接器）；对外上报实例/查询/触发检测的 REST API（API Token 鉴权）
2. **Security RBAC 收紧**：先做前端登录页 + token 存储/请求头，再收紧后端 `anyRequest().permitAll()`（当前全放行）

**M4**：
3. **3D 视图 + 大屏**：3d-force-graph 粒子流向、深色科技风投屏
4. **排查模式 3D 深化**、流向动画

**远期**：
5. PostgreSQL 实机验证（方言已就绪待实机）、数据池接 Oracle（新增方言+驱动）
6. 自动血缘解析、AI 辅助排查、告警渠道（钉钉/企微）、自动动作总开关与可回滚
7. 制造场景模板、开放 API 生态
...
**当前任务 = M3 剩余（CMDB 连接器 / 开放 API / Security RBAC）+ M4（3D / 大屏）**。M0~M2 收尾 + M3 择路核心均已交付并推送到 GitHub（后端 56 测试全绿）。
新会话第一步：读本文档 → `cd backend && mvn spring-boot:run` 起服务 → 从 CMDB 连接器或前端登录页 + RBAC 收紧开始。
前端已完整可用：http://localhost:5173（关系网画布+排查模式+路径查询+影响面、工作台看板、告警中心、检测点、配置版本、数据接入，全部真实数据）。
